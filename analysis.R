...
wdi <- filter(idata, source_id == "WDI_2015",
              variable %in% vars, unit %in% c("bn USD2005/yr", "million"))
...
ggplot() +
  ...
  # agriculture
  geom_point(data = df_plot, aes(x = gdp_pc, y = va_agr_pc_gr), colour = "green", alpha = plot_alpha) +
  # geom_smooth(data = df_plot, aes(x = gdp_pc, y = va_agr_pc_gr), method = lm, colour = "green") +
  # industry
  geom_point(data = df_plot, aes(x = gdp_pc, y = va_ind_pc_gr), colour = "blue", alpha = plot_alpha) +
  # geom_smooth(data = df_plot, aes(x = gdp_pc, y = va_ind_pc_gr), method = lm, colour = "blue") +
  # services
  geom_point(data = df_plot, aes(x = gdp_pc, y = va_ser_pc_gr), colour = "red", alpha = plot_alpha) +
  # geom_smooth(data = df_plot, aes(x = gdp_pc, y = va_ser_pc_gr), method = lm, colour = "red")
ggsave(filename = "plots/va_sec_pc_gr_over_gdp_pc.png", width = 14, height = 14, units = "cm")
...
# estimation
model_agr <- lm(va_agr_pc_gr ~ gdp_pc + I(gdp_pc^2) + factor(spatial) + recession, data = filter(wdi, spatial %in% g20))
model_ind <- lm(va_ind_pc_gr ~ gdp_pc + I(gdp_pc^2) + factor(spatial) + recession, data = filter(wdi, spatial %in% g20))
